...
const AppState = {
    _data: {
        ...
        searchSource: 'netease', // 🌟 新增变量：记录最后一次真实检索或切换的搜索源，防止关闭重开时UI重置
        searchResult: [],
        currentTrackIndex: -1,
        currentTrack: null,
        playMode: 'list',
...
const AppStore = {
    ...
    onStateMutation(key, val) {
        switch(key) {
            ...
            case 'isPlaying':
                ...
                const coverImg = document.querySelector('#currentSlideWrapper .artwork-img');
                if(val) { 
    ...
    async handleTrackViewTransition(track) {
        ...
        // 🌟 Bug 2 修复：优先使用歌曲对象自带的源，如果是搜索列表中还未保存到列表的歌，则使用 store.searchSource 锁定的源，不再盲目去找 'netease'
        let platform = track ? (track.platformSource || store.searchSource || 'netease') : 'netease';
        let targetCoverUrl = '';
        ...
        const triggerSlideAnimation = () => {
            ...
            const wrappers = viewport.querySelectorAll('.vinyl-slide-wrapper');
            wrappers.forEach(node => {
                if (node !== oldWrapper && node.id !== 'currentSlideWrapper') {
                    node.remove();
                }
            });
            ...
            if (oldWrapper) {
                ...
                oldWrapper.classList.remove('slide-active');
                oldWrapper.classList.add(store.lastDirection === 'next' ? 'slide-out-left' : 'slide-out-right');
            }

            const nextWrapper = document.createElement('div');
            nextWrapper.className = `vinyl-slide-wrapper ${store.lastDirection === 'next' ? 'slide-in-from-right' : 'slide-in-from-left'}`;
            ...
            nextWrapper.classList.remove('slide-in-from-right', 'slide-in-from-left');
            nextWrapper.classList.add('slide-active');
            ...
            setTimeout(() => {
                ...
                viewport.querySelectorAll('.vinyl-slide-wrapper').forEach(node => {
                    if (node !== nextWrapper) node.remove();
                });
                nextWrapper.id = 'currentSlideWrapper';
        ...
        nextCover.onload = function () { safeTrigger(); };
        nextCover.onerror = function () {
            this.onerror = null;
            if (this.src === DEFAULT_COVER) { safeTrigger(); return; }
            this.src = DEFAULT_COVER;
        };
    ...
    renderSheetLayout() {
        ...
        if (store.sheetTab === 'search') {
            ...
            // 🌟 Bug 1 修复：在渲染 select 时，通过检查 src === store.searchSource 来添加 'selected'，从而保留前次的搜索源状态
            const sourceOptions = store.allowedSources.map(src => `<option value="${src}" ${src === store.searchSource ? 'selected' : ''}>${SOURCE_DISPLAY_MAP[src] || src}</option>`).join('');
            body.innerHTML = `
                <div class="config-hub-panel"><div class="search-engine-box"><select class="custom-select-ui" id="platform" onchange="store.searchSource = this.value">${sourceOptions}</select>
    ...
    async executeCloudSearch() {
        ...
        store.searchSource = pf; // 🌟 搜索时同步锁定最新选择的源
    ...
    renderTrackNodes(list, isSearch, targetId) {
        ...
        list.forEach((track, i) => {
            ...
            const isCurrent = store.currentTrack && track.id === store.currentTrack.id && !isSearch;
            ...
            let artist = track.artist ? (Array.isArray(track.artist) ? track.artist.join(', ') : track.artist) : '未知';

            li.innerHTML = isSearch ? `
                <input type="checkbox" class="track-checkbox" id="cb-${i}" ${track.checked ? 'checked' : ''} onchange="AppStore.handleCheckboxChangeEvent(${i}, this.checked)">
            ...
            ` : `
                ...
                <div class="node-actions-cluster">
                    <button class="icon-btn danger-action" onclick="AppStore.mutateTrackNode(event, ${i}, false)">
    ...
    batchAddSelected() {
        ...
        let count = 0, localList = [...store.playlist], pf = store.searchSource; // 🌟 显式锁定当前的 searchSource
    ...
    mutateTrackNode(e, i, isSearch) {
        ...
        if(isSearch) {
            ...
            track.platformSource = store.searchSource; // 🌟 添加到歌单时，锁定歌曲的源为当前的 searchSource，不再跟随 select 的初始值
            localList.push(track);
    ...
    async playTrackByIndex(i, listGroup, closeSht = false, forcePause = false, isPreview = false) {
        ...
        // 🌟 Bug 2 修复：确定当前歌曲请求 URL 和 Lyric 时的源（如果是搜索列表试听，用 store.searchSource 兜底）
        let pf = targetTrack.platformSource || store.searchSource || 'netease';
        ...
        try {
            ...
            if (!uData || !uData.url) { this.toast('载入失败', '版权或音质不可用'); return; }
            ...
            if (isPreview) {
                ...
                targetTrack.platformSource = pf; // 🌟 试听时也将准确的源挂载到临时对象上，使封面转场能准确读取
            } else {
